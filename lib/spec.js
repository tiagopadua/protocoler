...
let EEmitter = require('./events.js');
let HexString = require('./hex-string.js');

const EVENT_ERROR = 'error';
const EVENT_VALUE = 'value';
...
class Spec extends EEmitter {
    constructor(inputJSON) {
        super();
        this._hex = null;
        this._specJSON = {};
        this._validateAndCopyJSON(inputJSON);
        this._registerEvent(EVENT_ERROR);
        this._registerEvent(EVENT_VALUE);
        this._registerEvent(EVENT_DESCRIPTION);
    }
    ...
    parseHex(inputHex) {
        this._hex = new HexString(inputHex);

        this.parseSpecList(this._specJSON.specList);

        let remaining = this._hex.getRemainingBytes();
        if (remaining !== null) {
            this.emit(EVENT_ERROR, 'There are remaining bytes at the end of the packet', remaining);
        }
    }
    ...
    parseSpecList(specList) {
        ...
        specList.every(spec => {
            ...
            if (typeof size === 'string') {
                size = parseInt('0x' + previousValues[size]);
            }
            let value = this._hex.getNextBytes(size);
            if (value === null) {
                this.emit(EVENT_ERROR, 'Not enough bytes in the packet', this._hex.getRemainingBytes());
                return false;  // break the loop
            }

            previousValues[spec.name] = value;

            this.emit(EVENT_VALUE, spec.name, value);

            this.parseValues(spec.values, value);

            return true;  // Do not break the loop
        });
    }

    parseValues(values, hexValue) {
        ...
        for (let value in values) {
            if (value === hexValue) {
                let valueObject = values[value];

                this.emit(EVENT_DESCRIPTION, valueObject.description);

                this.parseSpecList(valueObject.specList);
            }
        }
    }
}
