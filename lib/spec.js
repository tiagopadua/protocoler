...
class Spec {
    ...
    _validateAndCopySpecList(inputSpecList, localSpecList) {
        ...
        inputSpecList.every(singleSpec => {
            ...
            this._validateAndCopySingleSpec(singleSpec, localSingleSpec);
            return true;  // Do not break the loop
        });
    }

    _validateAndCopySingleSpec(inputSingleSpec, localSingleSpec) {
        ...
        for (let key in inputSingleSpec) {
            ...
            switch(key) {
                ...
                case 'size':
                    ...
                    if (typeof value === 'string' && !previousKeys.includes(value)) {
                        throw new SpecError('Value "' + value + '" should be a reference to a previous key, but there are none with this name.');
                    }
                    localSingleSpec[key] = value;
                    break;
                case 'values':
                    let localValue = {};
                    localSingleSpec[key] = localValue;
                    _validateAndCopySpecValues(value, localValue)
                    break;
                default:
