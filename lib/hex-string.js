...
class HexError extends EError {}

class HexString {

    constructor(inputString) {
        if (typeof inputString !== 'string') {
            throw new HexError('A single string parameter is required to create a new HexString instance.');
        }
        ...
        if (regexResult === null || !(regexResult instanceof Array) || regexResult.length < 3 || typeof regexResult[2] !== 'string') {
            throw new HexError('The input string should be a valid string formatted in hexadecimal, according the regex ' + hexRegex.toString());
        }
